...
def capture_and_highlight_object(object_name):
    ...
    command = ['libcamera-still', '-o', image_path, '--width', '1280', '--height', '720', '--nopreview', '--timeout', '1']
    result = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    
    if result.returncode != 0:
        print("Failed to capture image using libcamera-still")
...
def display_image(cv2image):
    ...
    # Check if a label to display the image exists
    if not hasattr(display_image, 'image_label'):
        # Create it if it doesn't exist
        display_image.image_label = tk.Label(display_image.capture_frame, image=imgtk)
        display_image.image_label.image = imgtk
        display_image.image_label.pack(expand=True)
    else:
        # Update the image if it does exist
        display_image.image_label.configure(image=imgtk)
        display_image.image_label.image = imgtk
